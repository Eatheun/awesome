...
local function worker(user_args)
	...
	local margin = args.margin or 30
	...
	function calendar_widget.toggle()
		local margins = { vertical = margin * 6, horizontal = margin }
		if popup.visible then
			...
			if placement == "top" then
				awful.placement.top(popup, { margins = { top = margins.vertical }, parent = awful.screen.focused() })
			elseif placement == "top_right" then
				awful.placement.top_right(popup, {
					margins = { top = margins.vertical, right = margins.horizontal },
					parent = awful.screen.focused(),
				})
			elseif placement == "top_left" then
				awful.placement.top_left(
					popup,
					{ margins = { top = margins.vertical, left = margins.horizontal }, parent = awful.screen.focused() }
				)
			elseif placement == "bottom_right" then
				awful.placement.bottom_right(popup, {
					margins = { bottom = margins.vertical, right = margins.horizontal },
					parent = awful.screen.focused(),
				})
			elseif placement == "bottom_left" then
				awful.placement.bottom_left(popup, {
					margins = { bottom = margins.vertical, left = margins.horizontal },
					parent = awful.screen.focused(),
				})
			else
				awful.placement.top(popup, { margins = { top = margins.vertical }, parent = awful.screen.focused() })
